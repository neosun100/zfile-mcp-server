...
### 💡 なぜ URL ベースのアップロード（Base64 ではなく）？

一部の MCP サーバーはファイルを base64 エンコードして LLM コンテキストに渡します — これはトークンとコストの無駄です：
- 1MB ファイル → 約35万トークンの無駄
- 5MB ファイル → 約175万トークンの無駄（ほとんどのコンテキスト制限を超過！）

**ZFile MCP Server はこの問題を完全に回避します。** サーバーはアップロード URL のみを AI クライアントに返し、ファイルは `curl` で直接 ZFile にアップロードされます。LLM を一切経由しません。結果：ファイルアップロードで **消費トークン 0**。
