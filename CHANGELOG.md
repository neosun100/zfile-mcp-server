...
- `get_base_url()` helper — Extracts server URL from `X-Forwarded-Host` / `X-Forwarded-Proto` headers
...
- `CHUNK_SIZE_MB` env var (default: 10MB per chunk)
...
| 📤 `zfile_upload` | Get upload URL (small files < 10MB) |
| 📦 `zfile_chunked_upload` | **NEW** Initialize chunked upload (large files > 10MB) |
...
|------|-------------|
| 📁 `zfile_list` | List files in directory |
| 📤 `zfile_upload` | Get upload URL for a file |
| 🔗 `zfile_direct_link` | Generate permanent direct link |
| 🔗 `zfile_short_link` | Generate 31-day short link |
